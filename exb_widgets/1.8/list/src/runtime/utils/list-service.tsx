...
export async function fetchSuggestionRecords (
  ...
  if (config.searchOpen && config.searchFields) {
    ...
    const dsCurrentQueryParams = (datasource as QueriableDataSource).getCurrentQueryParams()
    queryParams = (datasource as QueriableDataSource).mergeQueryParams(dsCurrentQueryParams, queryParams)
  }
...
async function queryRecords (q: any, ds: DataSource): Promise<any> {
  ...
  return await (ds as QueriableDataSource)
    .query(q, { scope: QueryScope.InConfigView })
