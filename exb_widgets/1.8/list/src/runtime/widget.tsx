...
export class Widget extends React.PureComponent<ListProps, ListStates> {
      ...
      >
        ...
        {(config?.layoutType === ListLayoutType.GRID) &&
          <Grid
            ...
            columnWidth={currentCardSize?.width + config?.horizontalSpace}
            rowCount={Math.ceil(this.records.length / this.getItemColumnCount()) || 1 }
            rowHeight={currentCardSize.height + config?.verticalSpace}
            width={listWidth}
            height={listHeight}
  ...
  getListMaxSize = () => {
    ...
    const maxheight = isListHeightCustom ? boundingBox?.height : this.bodySize.clientHeight
    ...
    if ((type as any) === 'ROW' && isContentHeightAuto && !isListHeightCustomUnitPX) {
      const viewportSizeHeight = viewportSize?.height || 800
      const maxHeight = appMode === AppMode.Design ? viewportSizeHeight : this.bodySize.clientHeight
  ...
  getListContentLeftPadding = () => {
    ...
    const { config } = this.props
    const listWidth = (widgetRect && widgetRect.width) || LIST_CARD_MIN_SIZE
    const rowWidth = this.getItemColumnCount() * (currentCardSize.width + config?.horizontalSpace)
    const padding = (listWidth - rowWidth) / 2 + config?.horizontalSpace / 2
    if (config?.layoutType === ListLayoutType.GRID) {
