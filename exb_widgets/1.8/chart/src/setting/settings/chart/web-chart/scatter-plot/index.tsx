...
import { IWebChart, WebChartSeries, ChartType, ChartDataSource, WebChartScatterPlotSeries } from '../../../../../config'
...
import { ScatterPlotAxes } from './axes'
import { ScatterPlotGeneral } from './general'
import { ScatterAppearance } from './appearance'
...
export const ScatterPlotSetting = (props: ScatterPlotSettingProps): React.ReactElement => {
  const {
    section,
  ...
  const handleTitleChange = (title: ImmutableObject<WebChartText>): void => {
    onWebChartChange?.(propWebChart.set('title', title))
  }

  const onFooterChange = (footer: ImmutableObject<WebChartText>): void => {
    onWebChartChange?.(propWebChart.set('footer', footer))
  }

  const handleLegendChange = (legend: ImmutableObject<WebChartLegend>): void => {
    onWebChartChange?.(propWebChart.set('legend', legend))
  }
    ...
    <>
      ...
      <SettingSection>
        ...
        >
          <SettingRow flow='wrap'>
            <ScatterPlotGeneral
              title={propWebChart?.title}
              footer={propWebChart?.footer}
              legend={propWebChart?.legend}
