...
const Setting = (props: SettingProps): React.ReactElement => {
  ...
  const handlePlaceholderTextChange = (text: string) => {
    const newPlaceholder = replacePlaceholderTextContent(placeholder, text?.trim())
    onSettingChange({
      id,
    ...
    <div className='widget-setting-text jimu-widget-setting'>
      ...
      <SettingSection>
        ...
        <SettingRow>
          <div className='w-100'>
            <ExpressionInput
              autoHide useDataSources={useDataSource && useDataSources} onChange={handleTooltipChange} openExpPopup={() => setOpenTip(true)}
