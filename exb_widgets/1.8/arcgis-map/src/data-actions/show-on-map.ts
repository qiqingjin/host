import {
  AbstractDataAction, DataSource, DataRecord, getAppStore, ImmutableObject, JimuMapViewInfo,
  MutableStoreManager, utils, i18n
} from 'jimu-core'
import { MapViewManager, SHOW_ON_MAP_DATA_ID_PREFIX, ShowOnMapDataType, zoomToUtils } from 'jimu-arcgis'
...
export default class ShowOnMap extends AbstractDataAction {
  ...
  async onExecute (dataSource: DataSource, records: DataRecord[], name, actionConfig): Promise<boolean> {
    ...
    if (jimuMapView) {
      const graphics = await jimuMapView.getGraphicsFromRecords(dataSource, records)
      if (graphics?.length > 0) {
        zoomToUtils.zoomTo(jimuMapView?.view, graphics, null)
      }
    }
